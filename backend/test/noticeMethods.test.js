...
describe('storeVerification', () => {
  test('Normal return', async () => {
    const user = await models.User.create({
      userName: 'name_storeVerification_noticeMethods',
      password: 'password_storeVerification_noticeMethods',
      email: 'name_storeVerification_noticeMethods'
    })
    const user2 = await models.User.create({
      userName: 'name2_storeVerification_noticeMethods',
      password: 'password_storeVerification_noticeMethods',
      email: 'name_storeVerification_noticeMethods'
    })
    const team = await models.Team.create({
      teamName: 'teamName_storeVerification_noticeMethods',
      founderID: user.id
    })
    const id = await noticeMethods.storeVerification({
      type: 'invitation',
      senderID: user.id,
      receiverID: user2.id,
      teamID: team.id
    })
    await noticeMethods.storeUserVerification({
      userID: user2.id,
      verificationID: id,
      hasSolved: NOT_READ
    })
...
describe('storeNotice', () => {
  test('Normal return', async () => {
    const user = await models.User.create({
      userName: 'name_storeNotice_noticeMethods',
      password: 'password_storeNotice_noticeMethods',
      email: 'name_storeNotice_noticeMethods'
    })
    const user2 = await models.User.create({
      userName: 'name2_storeNotice_noticeMethods',
      password: 'password_storeNotice_noticeMethods',
      email: 'name_storeNotice_noticeMethods'
    })
    const id = await noticeMethods.storeNotice({
      title: 'title_storeNotice_noticeMethods',
      content: 'contend_storeNotice_noticeMethods'
    })
    ...
    await models.Team.destroy({
      where: {
        teamName: 'teamName_storeNotice_noticeMethods',
        founderID: user.id
      }
    })
    await models.User.destroy({
